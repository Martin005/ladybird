...
ByteString extract_tld_hint(URL::URL const& url)
{
    ...
    auto host_string = url.host().has_value() ? url.serialized_host() : String {};
...
// https://html.spec.whatwg.org/multipage/parsing.html#determining-the-character-encoding
EncodingSniffResult run_encoding_sniffing_algorithm(DOM::Document& document, ReadonlyBytes input, Optional<MimeSniff::MimeType> maybe_mime_type)
{
    ...
    if (maybe_mime_type.has_value()) {
        ...
        if (maybe_transport_encoding != "invalid"sv)
            return { maybe_transport_encoding, false };
    }
    ...
    if (Parser::rust_detect_encoding(
        ...
        if (standardized.has_value())
            return { ByteString { standardized.value() }, true };
    }
